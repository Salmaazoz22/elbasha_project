...
| 17 | **Service descriptions** (10 × 1–2 sentences, Arabic and English) *(found during implementation; see M-09)* | Service cards | Still pending. The client planned draft descriptions (to be flagged "draft": true in services.json), but the texts were not included in batch 1. The client document asks for them as a single item together with material types. |
...
**Status (2026-09-15):** Phases 4–6 are complete; see `docs/QA_REPORT.md` and `docs/DEPLOYMENT_GUIDE.md`. §15 items 1, 3, 4, 5, 7, 8, 11, 15 and 16 are resolved (9 partly). Still **NEEDS CLIENT INPUT**: 2, 6, 9 (address, hours, profile), 10, 12, 13, 14, 17, 18, 19. The client-ready list is docs/CLIENT_REQUESTS.md (37 items after batch 1).
