...
| C-10 | **No trust signals** (clients, certifications, classification, years). **None exist in the project, so none will be added.** Slots are marked `[[NEEDS_CLIENT]]` only if the client wants them. → Clients list added 2026-09-18 (client batch 7), as text names only. | P1 (client) |
...
| 7 | **Scope wording:** bridge *construction* vs *painting*; what "Material Supply" covers | Hero headline accuracy (C-06) | ✅ **RESOLVED 2026-09-15 (client batch 1):** hero keeps the company name as headline in both languages; material types still pending (asked together with the service descriptions). ✅ **Material types RESOLVED 2026-09-19 (client batch 9, from the manager):** clay bricks and cement bricks (all sizes), interlock, curbstones, keystone blocks, crushed stone (all grades) and sand. Shown in the "We supply" strip below the home hero and named in the Material Supply description. |
...
| 10 | **Trust signals** (clients, certifications, classification, years) | None exist in the project | **Clients: ✅ RESOLVED 2026-09-18 (client batch 7):** 28 companies that agreed to be named, shown as text in «عملاؤنا وشركاؤنا» / "Our Clients & Partners" on the home and About pages (`src/data/clients.json`). No logos until a company sends its own file. Founding year: batch 3. Certificates and classification: not requested any more. |
...
**Status (2026-09-16):** Phases 4–6 are complete; see `docs/QA_REPORT.md` and `docs/DEPLOYMENT_GUIDE.md`. §15 items 1, 3, 4, 5, 7, 8, 9, 11, 15, 16 and 17 are resolved (9 and 17 in client batch 5, 2026-09-18). Still **NEEDS CLIENT INPUT**: 2, 6 (photos only; publishing permission granted in batch 5), 12, 13, 14, 18, 19, and the media items 20–23 added after the photo build. Client batch 6 (2026-09-18) added projects 29–32; project 30 came with photos (MEDIA_PROPOSAL §7). Client batch 7 (2026-09-18) supplied the clients & partners list (§15 item 10). The client-ready list is docs/CLIENT_REQUESTS.md (30 items after batch 7, the last two optional); the photo questions are in docs/photo-triage.pdf and docs/MEDIA_PROPOSAL.md §5.
